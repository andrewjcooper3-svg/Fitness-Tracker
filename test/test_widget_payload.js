...
(async () => {
  ...
  // Targets come off the plan, not a constant: Friday is now the rest day
  // (gym moved to Monday as of 2026-09-14), Sat is four sets, Sun none.
  const t = s.pushups.days.map(d => d.target);
  console.log('  targets Mon..Sun:', t.join(', '));
  check('Mon/Tue/Wed/Thu are each 165 (3 x 55)', t.slice(0, 4).every(x => x === 165), t.join(','));
  check('Friday is 0 - it is the rest day', t[4] === 0, String(t[4]));
